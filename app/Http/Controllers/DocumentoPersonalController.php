...
class DocumentoPersonalController extends Controller
{
    ...
    public function aprobar(Request $request, DocumentoPersonal $documento)
    {
        ...
        // Verificar permisos
        if (!$user->esSupervisor() && !$user->esAdministrador()) {
            return redirect()->back()->with('error', 'No tienes permisos para aprobar documentos.');
        }
        
        // Si es un cambio, rechazar el documento anterior
    ...
    public function rechazar(Request $request, DocumentoPersonal $documento)
    {
        ...
        // Verificar permisos
        if (!$user->esSupervisor() && !$user->esAdministrador()) {
            return redirect()->back()->with('error', 'No tienes permisos para rechazar documentos.');
        }
