    ...
    <div class="flex-1 lg:mr-64">
        ...
        <div class="container mx-auto px-4 py-6 max-w-4xl">
            <div class="flex items-center text-sm text-gray-600 mb-2">
                ...
                <span>Editar: {{ $punto->nombre }}</span>
            </div>

            <h1 class="text-3xl font-bold text-gray-800 mb-6">Editar punto de ronda</h1>

            <div class="bg-white rounded-lg shadow-md p-6">
                <form action="{{ route('admin.rondas.update', $punto) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <div class="mb-4">
                        <label for="nombre" class="block text-sm font-medium text-gray-700 mb-1">Nombre del punto <span class="text-red-500">*</span></label>
                        <input type="text" id="nombre" name="nombre" value="{{ old('nombre', $punto->nombre) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500" required>
                    ...
                    <div class="mb-4">
                        ...
                        <select id="sector_id" name="sector_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500">
                            ...
                            @foreach($sectores as $sector)
                                <option value="{{ $sector->id }}" {{ old('sector_id', $punto->sector_id) == $sector->id ? 'selected' : '' }}>{{ $sector->nombre }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="descripcion" name="descripcion" rows="2" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500">{{ old('descripcion', $punto->descripcion) }}</textarea>
                    </div>

                    <div class="mb-4">
                        <label for="orden" class="block text-sm font-medium text-gray-700 mb-1">Orden en ruta</label>
                        <input type="number" id="orden" name="orden" value="{{ old('orden', $punto->orden) }}" min="0" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500">
                    </div>

                    <div class="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-lg">
                        <p class="text-sm font-medium text-amber-800 mb-2">Ubicación del punto (obligatoria para validar escaneo)</p>
                        <p class="text-xs text-amber-700 mb-3">El escaneo solo será válido si el guardia está dentro del radio de proximidad configurado.</p>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label for="lat" class="block text-sm text-gray-700 mb-1">Latitud</label>
                                <input type="text" id="lat" name="lat" value="{{ old('lat', $punto->lat) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500" placeholder="Ej: -33.448890" inputmode="decimal">
                            ...
                            <div>
                                ...
                                <input type="text" id="lng" name="lng" value="{{ old('lng', $punto->lng) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500" placeholder="Ej: -70.669265" inputmode="decimal">
                        ...
                        <div class="mt-3">
                            ...
                            <input type="number" id="distancia_maxima_metros" name="distancia_maxima_metros" value="{{ old('distancia_maxima_metros', $punto->distancia_maxima_metros ?? 10) }}" min="1" max="500" class="w-32 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500">
                    ...
                    <div class="mb-6">
                        <label class="flex items-center">
                            <input type="checkbox" name="activo" value="1" {{ old('activo', $punto->activo) ? 'checked' : '' }} class="rounded border-gray-300 text-emerald-600 focus:ring-emerald-500">
                            <span class="ml-2 text-sm text-gray-700">Punto activo</span>
                        </label>
                    </div>

                    <p class="text-sm text-gray-500 mb-4">Código QR: <code class="bg-gray-100 px-1 rounded">{{ $punto->codigo }}</code> (no se modifica al editar)</p>
