        ...
        <div class="col-md-8">
            <div class="card border-danger">
                <div class="card-header bg-danger text-white">
                    <h4>Registrar: {{ \App\Models\ReporteEspecial::tipos()[$tipo] ?? 'Reporte' }}</h4>
                </div>
                <div class="card-body">
                    @if ($errors->any())
                    ...
                    <form method="POST" action="{{ route('reportes-especiales.store') }}" enctype="multipart/form-data" id="formReporte">
                        @csrf
                        ...
                        <div class="mb-3">
                            <label for="novedad" class="form-label">Novedad <span class="text-danger">*</span></label>
                            <textarea class="form-control @error('novedad') is-invalid @enderror" 
                                      id="novedad" 
                                      name="novedad" 
                                      rows="3" 
                                      required>{{ old('novedad') }}</textarea>
                            <small class="text-muted">Describa detalladamente la situación observada</small>
                            @error('novedad')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            <label for="accion" class="form-label">Acción Tomada <span class="text-danger">*</span></label>
                            <textarea class="form-control @error('accion') is-invalid @enderror" 
                                      id="accion" 
                                      name="accion" 
                                      rows="3" 
                                      required>{{ old('accion') }}</textarea>
                            <small class="text-muted">Describa qué acciones tomó frente a la situación</small>
                            @error('accion')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <small class="text-muted">Describa el resultado de las acciones tomadas</small>
                            @error('resultado')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            <label class="form-label">
                                ...
                                Agregar Fotos (Importante para evidencias)
                            </label>
                        ...
                        <div class="d-flex justify-content-between">
                            <a href="{{ route('reportes-especiales.index') }}" class="btn btn-secondary">Cancelar</a>
                            <button type="submit" class="btn btn-danger">Registrar Reporte</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        files.forEach((file, index) => {
            ...
            fileItem.innerHTML = `
                <div class="d-flex align-items-center">
                    <i class="fas fa-image text-danger me-3"></i>
                    <div>
